...
def plot_silhouette(silhouette_vals, y, 
    ...
    Args:
    silhouette_vals: nparray 
                     the silhouette values of the samples
    y:               nparray 
                     the labels of the samples
    colors:          color palette - default cm.tab10
    plot_noise:      boolean - default False
                     if true assumes the noise to be labeled with a negative integer
    """
    ...
    for i, c in enumerate(cluster_labels): # generate pairs index, cluster_label
        # plot horizontal bars for all the samples of one cluster, proportional to 
        # silhouette index, sorted for decreasing values
        ...
        y_ax_upper += len(c_silhouette_vals) # set the y upper value for the current cluster
        color = colors(i / n_clusters)
        plt.barh(range(y_ax_lower, y_ax_upper), c_silhouette_vals, height=1.0, 
                edgecolor='none', color=color) # plot the horizontal bars
        ...
        y_ax_lower += len(c_silhouette_vals) # set the y lower value for next cluster
